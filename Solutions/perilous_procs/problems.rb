...
def first_index(arr, &prc)
    arr.each.with_index { |ele, i| return i if prc.call(ele) }
    nil
end
...
def proctition_platinum(arr, *procs)
    ...
    procs.each_with_index do |prc, i|
        hash[i + 1] = []
        arr.each do |word|
            if prc.call(word) && !hash.values.flatten.include?(word) 
                hash[i + 1] << word
            end
        end
    end
...
def procipher(sen, hash)
    ...
    words.each_with_index do |w, i|  
        hash.keys.each do |prc|   
            if prc.call(w) 
                new_words[i] = hash[prc].call(new_words[i]) 
            end
        end
    end
    new_words.join(" ")
